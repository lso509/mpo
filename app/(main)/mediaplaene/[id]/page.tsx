...
function GanttChart({ plan, positions }: { plan: MediaplanRow; positions: PositionRow[] }) {
  ...
  const monthLabels = useMemo(() => {
    ...
    while (d.getTime() <= maxTime) {
      ...
      if (t >= minTime) {
        const left = ((t - minTime) / totalMs) * 100;
        if (left >= 0 && left <= 100) labels.push({ left, label: d.toLocaleDateString("de-CH", { month: "long" }) });
      }
          ...
          <div className="relative flex-1 min-w-0 flex flex-col">
            <div className="absolute inset-0 pointer-events-none z-10" aria-hidden>
              {monthBoundaries.map((leftPct, i) => (
                <div key={`month-${i}`} className="absolute top-0 bottom-0 w-px bg-zinc-300 dark:bg-zinc-500" style={{ left: `${leftPct}%` }} />
              ))}
            </div>
            <div className="relative z-0 mb-1 h-4">
              {monthLabels.map((m, i) => (
                <span key={i} className="absolute text-xs font-normal text-zinc-400 dark:text-zinc-500" style={{ left: `${m.center}%`, transform: "translateX(-50%)" }}>
                  {m.label}
                </span>
              ))}
            </div>
            <div className="relative z-0 mb-2 h-6">
              {weekLabels.map((w, i) => (
                <span key={i} className="absolute text-xs text-zinc-500 dark:text-zinc-400" style={{ left: `${w.center}%`, transform: "translateX(-50%)" }}>
                  {w.label}
                </span>
              ))}
            </div>
              ...
              {bars.map((bar) => (
                <div key={bar.id} className="relative h-8 rounded bg-zinc-100 dark:bg-zinc-700/50 overflow-hidden">
                  {weekBoundaries.map((leftPct, i) => (
                    <div key={`week-${i}`} className="absolute top-0 bottom-0 w-[2px] bg-white dark:bg-zinc-400 pointer-events-none" style={{ left: `${leftPct}%` }} aria-hidden />
                  ))}
                  <div
                    className="absolute top-1 bottom-1 rounded cursor-pointer"
                    style={{ left: `${bar.left}%`, width: `${bar.width}%`, backgroundColor: bar.color, opacity: 0.85 }}
                    title={`${bar.title}\nStart: ${bar.startDateLabel} – Ende: ${bar.endDateLabel}`}
                  />
                  {bar.hasCreativeDeadline && bar.creativeDeadlineDate && (
                    <div
                      className="absolute top-0 bottom-0 w-0.5 bg-amber-500"
                      style={{ left: `${((new Date(bar.creativeDeadlineDate).getTime() - min.getTime()) / totalMs) * 100}%` }}
