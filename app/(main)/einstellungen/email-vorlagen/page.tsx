...
export default function EmailVorlagenPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <header>
        <h1 className="text-3xl font-semibold tracking-tight text-zinc-950 dark:text-zinc-100">E-Mail Vorlagen</h1>
      ...
      <section className="haupt-box p-6 dark:bg-zinc-800">
        ...
        <div className="mt-4 flex gap-4">
          <input
            ...
            className="flex-1 rounded-lg border border-zinc-200 bg-white px-4 py-2 text-sm text-zinc-900 dark:border-zinc-600 dark:bg-zinc-800/80 dark:text-zinc-100"
          />
          <select className="rounded-lg border border-zinc-200 bg-white px-4 py-2 text-sm text-zinc-700 dark:border-zinc-600 dark:bg-zinc-800/80 dark:text-zinc-200">
          ...
          <ul className="mt-6 space-y-3">
            {templates.map((t) => {
              const created = t.created_at
                ? new Date(t.created_at).toLocaleDateString("de-CH", { day: "2-digit", month: "2-digit", year: "numeric" })
                : "";
              const updated = t.updated_at
                ? new Date(t.updated_at).toLocaleDateString("de-CH", { day: "2-digit", month: "2-digit", year: "numeric" })
                : null;
              ...
              return (
                <li key={t.id}>
                  <Link
                    ...
                    className="content-radius flex items-center justify-between border border-zinc-200 bg-white p-4 transition hover:border-[#FF6554]/50 dark:border-zinc-700 dark:bg-zinc-800/80 dark:hover:border-[#FF6554]"
                  >
                    <div className="flex items-start gap-3">
                      <span className="text-2xl opacity-70">✉</span>
                      <div>
                        <p className="font-semibold text-zinc-950 dark:text-zinc-100">{t.title}</p>
                        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">Betreff: {t.subject ?? "—"}</p>
      ...
      {newModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setNewModalOpen(false)}>
          <div
            className="content-radius w-full max-w-md border border-zinc-200 bg-white p-6 dark:border-zinc-700 dark:bg-zinc-900"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-xl font-semibold text-zinc-950 dark:text-zinc-100">Neue Vorlage erstellen</h3>
            <form className="mt-6 space-y-4">
              <div>
                <label className="block text-xs font-medium text-zinc-500">Name</label>
                <input type="text" className="mt-1 w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm" />
              </div>
              <div>
