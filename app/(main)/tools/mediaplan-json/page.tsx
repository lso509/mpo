...
export default function MediaplanJsonToolPage() {
  ...
  async function copyNormalizerPrompt() {
    ...
    setCopyBusy(true);
    try {
      const res = await fetch("/api/tools/mediaplan-import-prompt");
      if (!res.ok) {
        setCopyError(`Prompt konnte nicht geladen werden (${res.status}).`);
        return;
      }
      ...
      setCopyError(e instanceof Error ? e.message : "Kopieren fehlgeschlagen.");
    } finally {
      setCopyBusy(false);
    }
  }
    ...
    <div className="space-y-6 p-6">
      <header>
        ...
        <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
          Claude- bzw. KI-Export als neuer Mediaplan (Entwurf) mit Produkten und Positionen.
        </p>
      </header>
      ...
      <section className="haupt-box space-y-4 p-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
          <button
            type="button"
            disabled={copyBusy}
            onClick={copyNormalizerPrompt}
            className="inline-flex w-fit items-center rounded-full border border-zinc-200 bg-white px-5 py-2.5 text-sm font-medium text-zinc-800 hover:border-zinc-300 hover:bg-zinc-50 disabled:opacity-50 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-100 dark:hover:border-zinc-500 dark:hover:bg-zinc-800"
          >
            {copyBusy ? "Wird kopiert…" : copyDone ? "Kopiert!" : "Prompt für Excel → JSON kopieren"}
          </button>
          {copyError && (
        ...
        <h2 className="text-lg font-semibold text-zinc-950 dark:text-zinc-100">Format &amp; Ablauf</h2>
