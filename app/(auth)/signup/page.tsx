...
function SignupForm() {
  ...
  function getAuthBaseUrl() {
    return typeof process.env.NEXT_PUBLIC_APP_URL === "string" && process.env.NEXT_PUBLIC_APP_URL
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        emailRedirectTo: `${baseUrl}/auth/callback?next=${encodeURIComponent(redirect)}`,
        data: { role },
      },
    });
    setLoading(false);
    ...
    setMessage({
      type: "success",
      text: "Konto angelegt. Bitte E-Mail bestätigen (Link prüfen) oder direkt anmelden, falls Bestätigung deaktiviert ist.",
    });
  }
  ...
  return (
    <div className="w-full max-w-sm rounded-xl border border-zinc-200 bg-white p-6">
      ...
      <form onSubmit={handleSignup} className="mt-6 space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="mt-1 w-full rounded-lg border border-zinc-200 px-3 py-2 text-sm text-zinc-900"
          />
          <p className="mt-1 text-xs text-zinc-500">Mindestens 6 Zeichen</p>
        </div>
        <button
          type="submit"
          disabled={loading}
      ...
      <p className="mt-4 text-center text-sm text-zinc-500">
        Bereits Konto?{" "}
        <Link href={`/login${redirect !== "/dashboard" ? `?redirect=${encodeURIComponent(redirect)}` : ""}`} className="font-medium text-[#FF6554] hover:text-[#e55a4a]">
          Anmelden
        </Link>
      </p>
    </div>
